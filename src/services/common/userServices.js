...
const updateUserDetails = (public_id, patchDetail = []) => {
    return new Promise((resolve, reject) => {
        UserModel.findOne({ public_id: public_id }).select({ phone_number: 1, gender: 1 }).exec((err, foundUser) => {
            if (err) {
                reject({ err: err, status: 500, success: false })
            }
                ...
                UserModel.findOneAndUpdate({ public_id: public_id }, patchedDocument.newDocument, {
                    new: true, upsert: true,
                    select: {
                        password: 0, _id: 0,
                        createdAt: 0, updatedAt: 0, softDelete: 0,
                    }
                }).exec((error, patchedUser) => {
